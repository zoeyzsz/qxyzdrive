...
message_info = dict()
ignore = set()
@app.on_message(filters.command(['ts1']))
...
@app.on_message(filters.command(['hunter1']))
...
Example :
• /ts1 [Search Query]
• <code>/ts1 One Piece</code>
• <code>/ts1 Boruto</code>
...
Example :                              
• /hunter1 [Search Query]
• <code>/hunter1 SSNI-101</code>
• <code>/hunter1 HUNTA-965</code>
'''
...
SEARCHHELP_HANDLER = CommandHandler("torrent1", searchhelp)
